...
function App() {
	...
	const [isDark, setIsDark] = useState<boolean>(() => {
		const savedTheme = localStorage.getItem('brave-theme');
		if (savedTheme) {
	...
	const [language, setLanguage] = useState<string>(() => {
		return localStorage.getItem('brave-language') || 'ru';
	});

	useEffect(() => {
		localStorage.setItem('brave-theme', isDark ? 'dark' : 'light');
	...
	useEffect(() => {
		localStorage.setItem('brave-language', language);
	}, [language]);
